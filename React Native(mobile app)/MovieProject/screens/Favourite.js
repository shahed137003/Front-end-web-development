...
// Assuming you have a context for favourite movies
import { movieContext } from '../Context/MovieContextProvider';

export default function Favourite() {
  // Access context values properly
  const { favMovie,useFavMovie} = useContext(movieContext);
  // Function to delete a movie from favorites
  const deleteMovieFromFavorites = (movieId) => {
    const updatedMovies = favMovie.filter(movie => movie.id !== movieId); // Remove movie by id
    useFavMovie(updatedMovies); // Update the favMovie state
  };
  return (
    <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
      {/* Iterate through the favourite movie array from context */}
      {/* console.warn(favMovie) */}
      { favMovie.map((u, index) => (
        <View key={index} style={styles.container}>
          ...
          <View style={styles.containerButt}>
            <TouchableOpacity style={styles.button} onPress={() => deleteMovieFromFavorites(u.id)}>
